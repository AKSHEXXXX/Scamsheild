...
struct SubmissionHomeView: View {
    ...
    ScrollViewReader { proxy in
      ScrollView {
        VStack(alignment: .leading, spacing: SpacingTokens.large) {
          ...
          // Only show hero when no image is staged — reclaim space for the analyze flow
      ...
      // Sticky analyze bar — visible as soon as image is picked, no scrolling needed
      .safeAreaInset(edge: .bottom, spacing: 0) {
        if shouldShowAnalyzeBar {
          analyzeBar
        }
      }
      ...
      // Auto-scroll to preview the moment an image is picked
      .onChange(of: viewModel.previewImage) { image in
        if image != nil {
  ...
  private var shouldShowAnalyzeBar: Bool {
    guard viewModel.previewImage != nil else { return false }
    switch viewModel.state {
    case .loading, .success: return false
    default: return true
    }
  }
  ...
  private var sourceSection: some View {
    VStack(alignment: .leading, spacing: SpacingTokens.small) {
      if viewModel.previewImage != nil {
        // Image already staged — show a compact swap option instead of full source picker
        Button {
          viewModel.resetFlow()
          selectedPhotoItem = nil
        } label: {
          Label("Change image or source", systemImage: "arrow.triangle.2.circlepath")
            .font(TypographyTokens.caption)
