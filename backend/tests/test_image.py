...
@skip_if_no_supabase
@pytest.mark.anyio
async def test_sandbox_file_empty_bytes():
    """sandbox-file with empty/invalid base64 returns 400"""
    _ensure_token()
    transport = ASGITransport(app=app)
    async with AsyncClient(transport=transport, base_url="http://test") as client:
        resp = await client.post("/api/v1/sandbox-file", json={
            "filename": "test.txt",
            "file_bytes_b64": "aW52YWxpZA==",
            "os": "Android"
        }, headers=_headers)
