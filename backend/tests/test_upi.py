...
@skip_if_no_supabase
@pytest.mark.anyio
async def test_analyze_upi_safe():
    _ensure_token()
    transport = ASGITransport(app=app)
    async with AsyncClient(transport=transport, base_url="http://test") as client:
        resp = await client.post("/api/v1/analyze-upi", json={
            "amount": 500, "note": "lunch split", "vpa": "", "channel": "", "os": "Android"
        }, headers=_headers)
...
@skip_if_no_supabase
@pytest.mark.anyio
async def test_analyze_upi_returns_result():
    _ensure_token()
    transport = ASGITransport(app=app)
    async with AsyncClient(transport=transport, base_url="http://test") as client:
        resp = await client.post("/api/v1/analyze-upi", json={
            "amount": 1, "note": "share your OTP now", "vpa": "", "channel": "", "os": "Android"
        }, headers=_headers)
    assert resp.status_code == 200
    data = resp.json()
